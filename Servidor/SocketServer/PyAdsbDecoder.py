...
def ADSBDataDecoder(data, showStatus = True, dumpOnDatabase = True, latHome = -5.19506, lonHome = -39.28307):
    ...
    if len(data) == 28:
       ...
       DFCA = PyAdsbDecoderMathAndDataLibrary.toHex(data[:2]) #Da pra reduzir
       DF = PyAdsbDecoderMathAndDataLibrary.full_bit_zero(bin(eval(DFCA)))[:5] #Da pra reduzir
       DF = eval("0b" + DF) #Da pra reduzir
       CA = PyAdsbDecoderMathAndDataLibrary.full_bit_zero(bin(eval(DFCA)))[5:] #Da pra reduzir
       ...
       if DF == 17:
           ...
           if TC >= 1 and TC <= 4:
              ...
              if PyAdsbDecoderDatabase.VerifyAllPositionDataExists(ICAO) == True: #verifica exitencia dos dois dados e calcula rota
                   ...
                   rlat0 = 6. * (PyAdsbDecoderMathAndDataLibrary.modulo(j, 60.) + Airplanes1 / 131072.)
                   rlat1 = rlat1 = 6.101694915254237288 * (PyAdsbDecoderMathAndDataLibrary.modulo(j, 59.) + Airplanes2 / 131072.)
                   ...
                   NL0 = PyAdsbDecoderMathAndDataLibrary.NL(rlat0)
                   NL1 = PyAdsbDecoderMathAndDataLibrary.NL(rlat1)
                   ...
                   if F == '0':
                       ...
                       dlon = 360. / ni
                       rlon = dlon * (PyAdsbDecoderMathAndDataLibrary.modulo(m, ni) + Airplanes3 / 131072.)
                       Longitude = rlon
                       ...
                       dlon = 360. / ni
                       rlon = dlon * (PyAdsbDecoderMathAndDataLibrary.modulo(m, ni) + Airplanes4 / 131072.)
                       Longitude = rlon
