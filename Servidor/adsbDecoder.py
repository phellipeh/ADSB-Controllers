...
# - Colocar todas as referencias as bibliotecas
...
def ADSBDataDecoder(data):
    ...
    if DF == 17:
        if TC > 8 and TC < 19:
            # "Airborne Position message... Obtendo Dados (Altitude, Latitude e Longitude)"
            ...
            if adsbDecoderDatabase.VerifyAllPositionDataExists(ICAO) == True:
    ...
    if TC == 19:
	 ...
	 if (subtype == 1) { // ground non supersonic speed
	    ...
	    if ((!ew_dir) and (!ns_dir))
	       trk = (ew_spd == 0 ? 0 : 90 - 180. / M_PI * atan(ns_spd/ ew_spd));
					
	    if ((!ew_dir) and (ns_dir))
	       trk = (ew_spd == 0 ? 180 : 90 + 180. / M_PI * atan(ns_spd / ew_spd));

	    if ((ew_dir) and (ns_dir))
	       trk = (ew_spd == 0 ? 180 : 270 - 180. / M_PI * atan(ns_spd / ew_spd));

	    if ((ew_dir) adn (!ns_dir))
	       trk = (ew_spd == 0 ? 0 : 270 + 180. / M_PI * atan(ns_spd / ew_spd));
	  ...
	  '''
        ...
        adsbDecoderDatabase.UpdateAirplaneSpeedAndAngle(ICAO, Data)
